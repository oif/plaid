...
class AppSettings: ObservableObject {
    ...
    @Published var voiceMode: VoiceMode {
        didSet { 
            defaults.set(voiceMode.id, forKey: "voiceMode")
            if let data = try? JSONEncoder().encode(voiceMode) {
                defaults.set(data, forKey: "voiceModeData")
            }
        }
    }
    ...
    private init() {
        ...
        if let data = defaults.data(forKey: "voiceModeData"),
           let mode = try? JSONDecoder().decode(VoiceMode.self, from: data) {
            self.voiceMode = mode
        } else {
            self.voiceMode = .dictation
        }
    }
