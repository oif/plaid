...
class SherpaOnnxService {
    ...
    /// SenseVoice: "zh", "en", "ja", "ko", "yue", "auto" | Whisper: ISO 639-1 or "auto"
    static func mapLanguage(_ uiLanguage: String, for modelType: ModelType) -> String {
        let langCode = uiLanguage.lowercased()
    ...
    func initialize(with model: LocalModel, language: String = "auto") throws {
        let mappedLanguage = Self.mapLanguage(language, for: model.modelType)
        ...
        case .sensevoice:
            ...
            try initializeSenseVoice(modelPath: modelFiles.modelPath, tokensPath: modelFiles.tokensPath, language: mappedLanguage)
        ...
        case .whisper:
            ...
            try initializeWhisper(encoderPath: encoderPath, decoderPath: decoderPath, tokensPath: modelFiles.tokensPath, language: mappedLanguage)
        }

        currentModelType = model.modelType
        currentModel = model
        isInitialized = true
        print("SherpaOnnx: Initialized with model \(model.displayName), language: \(mappedLanguage)")
    }

    private func initializeSenseVoice(modelPath: String, tokensPath: String, language: String) throws {
    ...
    private func initializeWhisper(encoderPath: String, decoderPath: String, tokensPath: String, language: String) throws {
        var config = SherpaOnnxOfflineRecognizerConfig()
    ...
    func initializeAsync(with model: LocalModel, language: String = "auto") async throws {
        try await Task.detached(priority: .userInitiated) {
            try self.initialize(with: model, language: language)
        }.value
    }
